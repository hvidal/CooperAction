...
class SetaHeranca: public Connector
{
	public:
    SetaHeranca(int ConnId):Connector(ConnId)
    {
    }
...
void SetaHeranca::Paint(TCanvas *Board, TColor cor)
{
    if(!*Visible) return;
/*
//	Ponto A
...
*/
    ...
    PontoBase.X = p[0].x + 11 * cos(((Angulo)*M_PI)/180.0);
	PontoBase.Y = p[0].y - 11 * sin(((Angulo)*M_PI)/180.0);
...
void SetaHeranca::Salva(int Ordem, TIniFile *INITIAL)
{
    SalvaConector(Ordem, INITIAL);
}

void SetaHeranca::Carrega(int Ordem, TIniFile *INITIAL)
{
